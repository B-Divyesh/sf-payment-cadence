...
Generated with the factory Azure image model (`factory-image`) on 2026-08-28. Original commissioned output for this product. The chosen source and prompt sidecar live in `assets/src/`; responsive AVIF, WebP, and JPEG exports live in `public/assets/`. The logo and PWA icons are hand-authored SVG/PNG assets built from the same envelope-and-moon geometry.

The social preview at `public/assets/gentle-nudge-social.jpg` is a 1200×630 center crop of the reviewed commissioned landscape, made on 2026-09-06 with ImageMagick. It adds no text, people, brands, or new imagery. `public/icons/apple-touch-icon.png` is a 180×180 resize of the existing hand-authored PWA icon. Both inherit the original asset provenance above.
